...
import { bloodData as mockData, } from './mock_data/data'
    ...
    this.operator = [
      {
        ...
        icon: (
          <Tooltip title="展开/收缩血缘">
            <BorderOuterOutlined style={{marginRight: '20px'}} />
          </Tooltip>
        ),
        onClick: (nodeData) => {
          // 展开血缘
      ...
      // },
      {
        ...
        icon: (
          <Tooltip title="折叠">
            <CloseCircleOutlined style={{marginRight: '20px'}} />
          </Tooltip>
        ),
        onClick: (nodeData) => {
          ...
          if (this.state.flag) {
            ...
            children.relations.forEach((relation) => {
              const index = newData.relations.findIndex(
                (r) => r.id === relation.id
              )
